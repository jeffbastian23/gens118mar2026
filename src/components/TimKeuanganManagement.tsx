...
export default function TimKeuanganManagement() {
  const [members, setMembers] = useState<TimKeuanganMember[]>([]);
  const [loading, setLoading] = useState(true);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [deleteId, setDeleteId] = useState<string | null>(null);
  const [editMember, setEditMember] = useState<TimKeuanganMember | null>(null);
  ...
  const fetchMembers = async () => {
    try {
      ...
      setMembers((data as TimKeuanganMember[]) || []);
    } catch (error: any) {
      toast.error("Gagal memuat data Tim Keuangan");
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editMember) {
        ...
        const { error } = await supabase.from("tim_keuangan").insert({
          name: formData.name,
          email: formData.email,
          telepon: formData.telepon || null,
          tugas: formData.tugas || null,
        ...
        if (error) throw error;
        toast.success("Anggota Tim Keuangan berhasil ditambahkan");
      }
  ...
  const openEditDialog = (member: TimKeuanganMember) => {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-semibold">Kelola Tim Keuangan</h3>
        </div>
            ...
            {members.length === 0 ? (
              <TableRow>
                <TableCell colSpan={8} className="text-center text-muted-foreground py-8">
                  Belum ada anggota Tim Keuangan
                </TableCell>
              </TableRow>
            ) : (
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editMember ? "Edit Anggota Tim Keuangan" : "Tambah Anggota Tim Keuangan"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Hapus Anggota Tim Keuangan?</AlertDialogTitle>
            <AlertDialogDescription>
